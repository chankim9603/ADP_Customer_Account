...
public class Registration {
    ...
    @PostMapping
    public ResponseEntity<?> regNewCustomer(@RequestBody Customer newCustomer, UriComponentsBuilder uri){
        if (newCustomer.getId() != 0 || newCustomer.getName() == null || newCustomer.getEmail() == null) {
			return ResponseEntity.badRequest().build();
		}

        String cust_json_string = getCustomerAsJSONString(newCustomer);

        sendNewCustDataToCustomerAPI(cust_json_string);
